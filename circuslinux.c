...
typedef enum {
	...
	STATE_TITLE,
	STATE_GAME,
	STATE_GAME_WAITING_READY,
	STATE_GAME_READY,
	STATE_GAME_PAUSE,
	STATE_GAMEOVER,
	STATE_ASK_FOR_INITIALS,
	STATE_HIGHSCORES,
	STATE_QUIT
} GameState;
#pragma endregion
...
int use_sound, use_fullscreen, use_low, use_joystick, use_joy_analog,
		...
		has_highscore, highscore_index, show_highscores = 0, highscore_effect, frame = 0;
SDL_Surface *screen;
...
Uint32 last_time;
int i, done = 0, quit = 0, any, some, player, background_frame = 0,
...
SDL_Keycode key;
GameState status = STATE_INITIAL;
GameState prev_status = STATE_INITIAL;
...
void game(void) {
	...
	if (status == STATE_GAME || status == STATE_GAME_WAITING_READY || status == STATE_GAME_READY) {
	...
	if (prev_status != STATE_GAMEOVER && status == STATE_GAMEOVER) {
		/* Ungrab mouse: */
	...
	if (status == STATE_ASK_FOR_INITIALS) {
		frame++;
...
int done, quit, i, j, tmp, text_x, text_xm, text_img, text_time,
	...
	while (SDL_PollEvent(&event)) {
		if (event.type == SDL_QUIT) {
			...
			if (key == SDLK_SPACE || key == SDLK_TAB ||
				...
				status = STATE_GAME_WAITING_READY;
...
void main_loop() {
	switch (status) {
		...
		case STATE_GAME:
			...
			status = STATE_GAME_WAITING_READY;
			break;
		case STATE_ASK_FOR_INITIALS:
		case STATE_GAMEOVER:
		case STATE_GAME_WAITING_READY:
